...
def create_app() -> FastAPI:
    ...
    async def dashboard(request: Request):
        ...
        try:
            ...
            return HTMLResponse(
                content=(
                    "<!DOCTYPE html><html><head><title>Patchbay</title></head>"
                    '<body style="background:#0e1117;color:#e6edf3;font-family:sans-serif;'
                    "display:flex;align-items:center;justify-content:center;height:100vh;"
                    'margin:0"><p>Please log in through your identity provider.</p>'
                    "</body></html>"
                ),
                status_code=401,
            )

        # Build service statuses for initial render (filtered by view permission)
        service_statuses = []
        ...
        # Build preset data (filtered by view permission)
        visible_presets = [p for p in config.presets if can_view(auth_ctx, p, auth_config)]
        presets_data = [
            {
                "name": p.name,
        ...
        return templates.TemplateResponse(
            "index.html",
            {
                ...
                "version": __version__,
                "init_data": init_data,
                "auth_username": auth_ctx.username,
            },
        )
